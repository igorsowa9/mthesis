...
figure(1)
plot(H,ZabsM3, 'b', 'LineWidth', 2);
axis(view)
title('Frequency Sweep: 3. case (4 WTs). Seen from middle LCL.');
xlabel(xlab);
ylabel(ylab);

figure(5)
semilogy(H,ZabsM3, 'b', 'LineWidth', 2);
return
...
plot(H,ZmodalAllM);
axis(view);
title('Harmonics Modal Analysis (ALL modes): 3. case (4 WTs)');
xlabel(xlab);
ylabel(ylab);
...
plot(H,ZmodalMaxM(:,3));
axis(view)
title('Harmonics Modal Analysis (for only MAX impedances): 3. case (4 WTs)');
xlabel(xlab);
ylabel(ylab);
...
title('Harmonics Modal Analysis (Critical modes only): 3. case (4 WTs)');
